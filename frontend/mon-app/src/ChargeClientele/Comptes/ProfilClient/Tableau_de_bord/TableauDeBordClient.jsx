...
const TableauDeBordClient = (props) => {
  ...
  useEffect(()=>{
    ...
    axios.get("http://localhost:4000/api/cdc/user/"+props.match.params.id).then((compte) => {
      setProfil(compte.data);
      console.log(sateProfil)
    });
    axios
      .get("http://localhost:4000/api/cdc/dernier/envoyer/"+props.match.params.id)
      .then((his) => {
        setVirementEnvoyer(his.data);
      });

    axios.get("http://localhost:4000/api/cdc/dernier/recu/"+props.match.params.id).then((his) => {
      setVirementRecu(his.data);
    });

    axios.get("http://localhost:4000/api/cdc/user/stat/"+props.match.params.id).then((fact)=>{
